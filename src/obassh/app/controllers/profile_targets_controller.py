...
class ProfileTargetsController:
    ...
    def __init__(
        ...
        app: App[str | None],
        state: AppState,
        inventory: OciInventoryProvider,
    ) -> None:
        self._app = app
        self._state = state
    ...
    def _update_status(self, message: str) -> None:
        self._app.query_one("#session-selection", Static).update(message)

    def selected_profile(self) -> OciProfileRef | None:
        return next(
            (p for p in self._state.profiles if p.name == self._state.selected_profile_name),
            None,
        )
    ...
    def ensure_single_bastion(self, profile_name: str) -> bool:
        ...
        try:
            ...
            self._update_status(f"Failed loading bastions: {exc}")
            return False
        if len(bastions) != 1:
            self._update_status(f"Expected one bastion, found {len(bastions)}")
            return False
        self._state.selected_bastion_ocid = bastions[0]["ocid"]
        self._app.query_one("#settings-bastion-ocid", Input).value = (
    ...
    def load_profiles(self) -> None:
        ...
        for profile in self._state.profiles:
            profiles_table.add_row(
                profile.name,
                profile.region,
                profile.tenancy_ocid,
                profile.compartment_ocid,
            )
    ...
    def load_targets_for_profile(self, profile_name: str) -> None:
        ...
        if not compartment_id:
            self._app.query_one("#targets-selection", Static).update(
                "No compartment set. Use COMPID env var or .oci/oci_cli_rc"
            )
            return
